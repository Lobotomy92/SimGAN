...
real_image_stack = np.load("../real.npy")
...
def refiner_network(input_image_tensor):
    ...
    def resnet_block(input_features, nb_features=64, nb_kernel_size=3):
        ...
        """
        y = layers.convolutional.Conv1D(nb_features, nb_kernel_size, padding='same')(input_features)
        y = layers.Activation('relu')(y)
        y = layers.convolutional.Conv1D(nb_features, nb_kernel_size, padding='same')(y)

        y = layers.Add()([input_features, y])
        return layers.Activation('relu')(y)

    # an input image of size w × h is convolved with 3 × 3 filters that output 64 feature maps
    x = layers.convolutional.Conv1D(64, 3, padding='same', activation='relu')(input_image_tensor)
    ...
    return layers.convolutional.Conv1D(img_channels, 1, padding='same', activation='tanh')(x)
...
def discriminator_network(input_image_tensor):
    ...
    """
    x = layers.convolutional.Conv1D(96, 3, padding='same', strides=2, activation='relu')(input_image_tensor)
    x = layers.convolutional.Conv1D(64, 3, padding='same', strides=2, activation='relu')(x)
    x = layers.MaxPooling1D(pool_size=3, padding='same', strides=1)(x)
    x = layers.convolutional.Conv1D(32, 3, padding='same', strides=1, activation='relu')(x)
    x = layers.convolutional.Conv1D(32, 1, padding='same', strides=1, activation='relu')(x)
    x = layers.convolutional.Conv1D(2, 1, padding='same', strides=1, activation='relu')(x)
